...
class Alignment:
    ...
    def get_cigar_tuples(self) -> list[tuple[CigarOperation, int]]:
        return [(CigarOperation(op[0]), op[1]) for op in self.alignment.get_cigar_tuples()]
    ...
    def get_first_aligned_exon_range(self) -> tuple[int, int]:
        ...
        head_soft_clipping_length = self.alignment.get_query_alignment_start()
    ...
    def get_second_aligned_exon_range(self) -> tuple[int, int]:
        ...
        query_sequence_length = self.alignment.get_query_sequence_length()
        query_alignment_end = self.alignment.get_query_alignment_end()
    ...
    def get_first_aligned_exon(self):
        ...
        return self.alignment.get_query_sequence()[first_aligned_exon_range[0]:first_aligned_exon_range[1]]
    ...
    def get_second_aligned_exon(self):
        ...
        return self.alignment.get_query_sequence()[second_aligned_exon_range[0]:second_aligned_exon_range[1]]
    ...
    def get_intron_range(self) -> tuple[int, int]:
        reference_start = self.alignment.get_reference_start()
    ...
    def process_alignment(self, reference : Reference) -> list:
        ...
        return [
            self.id,
            self.alignment.get_cigar_string(),
            self.alignment.get_query_sequence(),
            reference.get_slice(self.alignment.get_reference_start(), self.alignment.get_reference_end()),
            first_aligned_exon,
            intron,
            second_aligned_exon,
            len(intron),
